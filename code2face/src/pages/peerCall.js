...
const PeerCall = () => {
  ...
  async function detectFaceMotions() {
    ...
    timer = setInterval(async () => {
      ...
      if (userMoves > 12) {
        toast.error(
          'Warning! Interviewer will be notified if movement is observed again.'
        );
  ...
  useEffect(() => {
    ...
    const init = async (videoStream) => {
      ...
      peer.on('open', function (id) {
        ...
        if (interviewer) detectFaceMotions();
  ...
  const copyCode = (e) => {
    e.preventDefault();
    if (copy(roomId)) toast.success('Session ID copied');
      ...
      <div className="callpage d-flex flex-wrap">
        <div className="vcont">
          ...
          <div className="options d-flex flex-wrap">
            ...
            >
              {!displayCheck ? 'Screen Share' : 'Stop Share'}
            </Button>
            <Button
              onClick={copyCode}
              className="mt-2 btn-info"
              style={{ width: '160px', margin: 'auto', marginLeft: '20px' }}
            >
              Copy Session Id
            </Button>
            <div className="chat-toggler" onClick={chatHider}>
              {!chatState ? <img src="/toggle_chat.png"></img> : <></>}
            </div>
          </div>
        </div>
